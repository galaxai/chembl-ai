...
- Airflow UI: `8081` (container `8080`)

## Run Docker Compose directly
Use the checked-in compose file: `spark_airflow/DockerCompose.yml`.
...
docker compose -f spark_airflow/DockerCompose.yml up -d
```

Stop:

```bash
docker compose -f spark_airflow/DockerCompose.yml down
```
